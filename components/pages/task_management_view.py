# /components/pages/task_management_view.py
...
def TaskManagementView(page: Page, case_id: int):
    ...
    # 💡 修正: pick_date() は廃止されたため、page.open() を使用する方式に変更
    # （overlayへのappendも page.open を使う場合は不要です）

    def open_date_picker(e):
        page.open(date_picker)  # 💡 修正箇所
        page.update()
        ...
        for t in tasks:
            ...
            def on_check_change(e, task_id=t["task_id"]):
                ...
                if current_task:
                    ...
                    # イベント発生時は既に表示済みなので update_ui=True (デフォルト)
                    load_tasks()
    ...
    def save_dialog_data():
        ...
        if success:
            close_dialog()
            # 保存後は画面更新が必要なので update_ui=True (デフォルト)
            load_tasks()
    ...
    # 初期ロード時はまだ画面に追加されていないので update_ui=False を指定
    ...
    return Column(
        controls=[
            Row(
                [
                    Text("📋 タスク管理", size=24, weight="bold", color=Colors.WHITE),
                    ElevatedButton(
                        "＋ タスク追加",
                        icon=Icons.ADD,
