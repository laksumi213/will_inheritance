...
def create_contact_input_row(
    ...
    def remove_row(e):
        if row in column_container.controls:
            ...
            # エラーを回避するために、既に削除されている場合は何もしない
            print(
                f"Warning: Attempted to remove a row that was already removed. Row ID: {id(row)}"
            )
...
def add_initial_contact_rows(column_container: Column, is_email: bool, count: int = 1):
    """指定数（デフォルト1）の空行を追加する（クリアは呼び出し元で実施）"""
...
def add_new_contact_row(e, column_container: Column, is_email: bool):
    ...
    new_row, new_field = create_contact_input_row(
        column_container, is_email=is_email, initial_sub_type=""
    )
    column_container.controls.append(new_row)
    column_container.update()
    new_field.focus()
...
def copy_to_clipboard_and_notify(e, page: Page):
    """クリックされたテキストをクリップボードにコピーし、SnackBarで通知する"""
    # TextがContainerにラップされていることを前提に、Containerのdata属性からテキストを取得
    # もしくはイベントコントロールのdata属性から取得するように、update_heirs_listでdataにコピー対象を設定する

    # データを取得
    text_to_copy = e.control.data.strip()
...
def DeceasedDetailView(page: Page, deceased_id: int):
    ...
    def save_dialog(is_deceased: bool):
        ...
        print("--- [DEBUG] 収集された連絡先 ---")
        print(f"  電話番号リスト: {all_phone_contacts}")
        print(f"  メールアドレスリスト: {all_email_contacts}")
        print("---------------------------------")
            ...
            if deceased_id > 0:
                ...
                if registered_deceased:
                    ...
                    if contracting_heir:
                        ...
                        # --- 契約者連絡先 (追加された部分) ---
                        contact_info = deceased_service.get_contact_info(
                            "heir", contracting_heir.id
                        )
                        phone_contacts = [
                            c for c in contact_info if c["type"] == "PHONE"
                        ]
                        email_contacts = [
                            c for c in contact_info if c["type"] == "EMAIL"
                        ]
                        ...
                        if phone_contacts:
                            ...
                            for contact in phone_contacts:
                                print(
                                    f"    - [{contact['sub_type'] or '未設定'}]: {contact['value']}"
                                )
                        else:
                            print("  電話番号: N/A")
    ...
    def open_deceased_dialog(e):
        ...
        # 連絡先情報を取得
        contacts = deceased_service.get_contact_info("deceased", deceased_id)
        ...
        # phone_inputs_columnのcontrolsに変更を加えたため、明示的に更新を要求
        phone_inputs_column.update()
        ...
        # email_inputs_columnのcontrolsに変更を加えたため、明示的に更新を要求
        email_inputs_column.update()
    ...
    def open_heir_dialog(e, heir_id: int):
        ...
        if heir_to_edit:
            ...
            # 電話
            phone_contacts = [c for c in contacts if c["type"] == "PHONE"]
            phone_inputs_column.controls.clear()  # 既存のコントロールをクリア
            if phone_contacts:
                for c in phone_contacts:
                    new_row, _ = create_contact_input_row(
                        phone_inputs_column,
            ...
            # メール
            email_contacts = [c for c in contacts if c["type"] == "EMAIL"]
            email_inputs_column.controls.clear()  # 既存のコントロールをクリア
            if email_contacts:
                for c in email_contacts:
                    new_row, _ = create_contact_input_row(
                        email_inputs_column,
            ...
            wareki_dob_text.value = convert_seireki_to_wareki(
                heir_to_edit.date_of_birth
            )
            wareki_dod_text.value = convert_seireki_to_wareki(
                heir_to_edit.date_of_death
            )
    ...
    def update_heirs_list():
        ...
        for heir in current_deceased.heirs:
            ...
            primary_address = raw_address or "N/A"

            building = address_info.get("building_name", "")
            if building:
                primary_address += f" ({building})"

            phone_display_text = f"電話: {primary_phone}"
            ...
            heirs_controls.controls.append(
                Row(
                    [
                        ...
                        Text(f"続柄: {heir.relationship_type}", width=100),
                        # ★ 電話番号 (クリックでコピー) ★
                        Container(
                            content=Text(
                                phone_display_text,
