...
def start_mizuho_reservation_process(page: Page, case_id: int):
    ...
    if not data:
        page.open(
            SnackBar(
                content=Text(
                    f"エラー: データベースからみずほ銀行（{target_bank_code}）の予約に必要なデータが見つかりませんでした。",
                    color=Colors.WHITE,
                ),
    ...
    def run_automation():
        try:
            ...
            page.open(
                SnackBar(
                    content=Text(
                        "✅ みずほ銀行来店予約のWeb操作が完了しました。", color=Colors.WHITE
                    ),
...
class MizuhoReservation:
    def __init__(self, page: Page, data: dict):
        ...
        self.deceased_dob = (
            self.data["deceased_dob"].split("-")
            if self.data["deceased_dob"]
            else ["0000", "00", "00"]
        )
    ...
    def click_button_by_text(self, driver, text):
        ...
        try:
            ...
            raise NoSuchElementException(
                f"エラー: ボタン '{text}' が見つからないか、クリックできませんでした。"
            )
    ...
    def reservation(self):
        ...
        WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.NAME, "reserveform")))

        ## 1. 必須のチェックボックス（3項目）の操作
        driver.find_element(
            By.XPATH, "//*[@id='right-column']/div[1]/form/div[1]/div[2]/div[1]/label/span"
        ).click()
        ...
        # 各種証明書発行
        driver.find_element(
            By.XPATH, '//*[@id="right-column"]/div[1]/form/div[1]/div[6]/div[2]/label/span'
        ).click()

        # ご予約時刻を15分過ぎても... -> 確認しました
        driver.find_element(
            By.XPATH, '//*[@id="right-column"]/div[1]/form/div[1]/div[8]/div/label/span'
        ).click()
        ...
        driver.find_element(By.NAME, "attr_org1").send_keys(
            f"{self.staff_name_kanji}（{self.data['case_number']}）"
        )

        # ご来店者のお名前【全角カナ】 (DB固定値)
        driver.find_element(By.NAME, "attr_org2").send_keys(
            self.data["staff_name_kana"]
        )  # カタカナ変換が必要だが、ここでは仮に漢字の読み仮名を使用
        ...
        # 連絡がつきやすい時間帯 (いつでも)
        driver.find_element(
            By.XPATH, "//*[@id='right-column']/div[1]/form/div[1]/div[12]/div[1]/label/span"
        ).click()
        ...
        Select(driver.find_element(By.ID, "bt_form_cus_birthm")).select_by_value(
            self.firm_dob_month
        )
        Select(driver.find_element(By.ID, "bt_form_cus_birthd")).select_by_value(self.firm_dob_day)

        # 郵便番号 (法人郵便番号)
        driver.find_element(By.NAME, "cus_zip").send_keys(
            mojimoji.zen_to_han(self.firm_zip or "1030028")
        )
        driver.find_element(
            By.XPATH, '//*[@id="right-column"]/div[1]/form/div[1]/div[18]/input[2]'
        ).click()
        sleep(1)  # 住所自動入力の待機
        ...
        # 店番号 (DBから取得した支店コード)
        driver.find_element(By.NAME, "attr_org4").send_keys(
            mojimoji.zen_to_han(self.bank_branch_code or "0")
        )

        # 口座番号 (DBから取得した口座番号)
        driver.find_element(By.NAME, "attr_org6").send_keys(
            mojimoji.zen_to_han(self.bank_account_number or "0")
        )

        # ご予約時に選択いただいたメニュー... -> 確認しました
        driver.find_element(
            By.XPATH, '//*[@id="right-column"]/div[1]/form/div[1]/div[34]/div/label/span'
        ).click()

        # 個人情報の利用目的... -> 同意します
        driver.find_element(
            By.XPATH, '//*[@id="right-column"]/div[1]/form/div[1]/div[36]/div/label/span'
        ).click()

        # メールマガジン -> 配信を希望しない
        driver.find_element(
            By.XPATH, '//*[@id="right-column"]/div[1]/form/div[1]/div[38]/div[2]/label/span'
        ).click()
        ...
        self.page.show_snack_bar(
            SnackBar(
                content=Text(
                    "✅ 最終確認画面に遷移しました。Webブラウザで「予約内容を確認」をクリックして予約を完了してください。",
                    color=Colors.WHITE,
                ),
                bgcolor=Colors.GREEN_700,
            )
