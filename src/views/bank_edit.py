...
class BankMasterEditDialog(AlertDialog):
    ...
    def show(self, bank_id: int | None = None):
        ...
        if bank_id is not None:
            db = SessionLocal()
            try:
                bank = get_bank_master_by_id(bank_id) # サービス関数は内部でSessionを持つか引数で渡すか要確認。ここではDeceasedServiceの定義に準拠
                # 注意: get_bank_master_by_id がサービスで定義されていない場合は直接クエリする
                if not bank:
...
class BankEditView(Column):
    ...
    def load_bank_options(self):
        ...
        banks = get_bank_masters() # Service call handles Session
        options = [
    ...
    def load_account_type_options(self):
        """口座種類マスタをロード"""
        types = get_account_type_masters() # Service call handles Session
        options = [
            dropdown.Option("def", text="選択してください"),
            dropdown.Option("add_new_type", text="[ ➕ 新しい口座種類を登録 ]"),
        ]
        for t in types:
    ...
    def load_branch_options(self, bank_id: int):
        ...
        branches = get_branch_masters_by_bank_id(bank_id) # Service call handles Session
    ...
    def save_asset(self, e):
        try:
            ...
            if self.editing_asset_id:
                ...
                # 新規登録時は add_financial_asset_with_type を使用し、BANKを指定
                success = add_financial_asset_with_type(
                    case_id=self.case_id,
                    asset_type="BANK", # 銀行として登録
                    bank_id=bank_id, 
                    branch_id=branch_id, 
                    account_type_id=account_type_id,
                    account_number=self.account_number_field.value,
