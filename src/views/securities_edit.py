...
# ---------------------------------------------
# 部店マスタ (BranchMaster) 編集/新規登録用モーダル
# ---------------------------------------------
...
def save_branch_master(e, page: Page):
    ...
    if not securities_name_field.value:
        return # Error check done in open_branch_dialog
    ...
    try:
        # サービス呼び出し (DBロジックはサービスへ移譲)
        new_branch = add_branch_master(parent_bank_id, name, code)
        ...
        # 重複エラー等
        page.open(SnackBar(Text(f"エラー: {ex}"), bgcolor=Colors.RED_500))
        page.update()
...
def save_master(e, page: Page):
    ...
    # サービス経由で保存
    new_master = add_or_update_bank_master(current_editing_master_id, name, code)
...
def load_securities_options(page: Page):
    """
    DBから証券会社リストを取得してドロップダウンに設定。
    """
    # サービスから取得 (DB操作はサービス内で完結)
...
def update_branch_details(page: Page):
    ...
    if selected_str and selected_str != "def":
        try:
            # サービス層はID検索のAPIがないため、リストから探すか、簡易的に実装するか
            # ここではサービス経由ではなく、リストロード時に保持している情報を使うのが効率的だが
            # 構造上、サービスに追加せずに既存の get_branch_masters_by_bank_id を活用しにくいので
            # 簡易的にリストからマッチングするロジックにするか、またはサービス追加する。
            # 今回は既存リストを活用する。
            
            # (注) BranchMaster個別の取得関数がないため、ここで実装する場合は
            # load_branch_options で保持しているデータを使う必要があるが、
            # ステートレスにするため、親IDから再検索する。
            bank_id_str = securities_name_field.value
            if bank_id_str and bank_id_str not in ["def", "add_new"]:
                branches = get_branch_masters_by_bank_id(int(bank_id_str))
                selected_branch = next((b for b in branches if str(b.id) == selected_str), None)
                if selected_branch:
                    branch_code_field.value = selected_branch.branch_code
                    branch_code_field.update()
        except ValueError:
            pass
