...
class CaseDashboardView(Column):
    ...
    def _show_snack(self, msg, color):
        self.page.open(SnackBar(content=Text(msg, color=Colors.WHITE), bgcolor=color, duration=3000))
        self.page.update()
    ...
    def _on_export_result(self, e: FilePickerResultEvent):
        if e.path:
            ...
            self._show_snack(f"保存{'成功' if success else '失敗'}: {e.path}", Colors.GREEN if success else Colors.RED)
    ...
    def _on_import_result(self, e: FilePickerResultEvent):
        if e.files:
            ...
            if success: self._update_all_views()
    ...
    def _get_case_items(self, search_term="", status_filter_id=None, user_filter_id=None):
        ...
        if not cases:
            ...
            return [Container(content=Text(msg, color="onSurfaceVariant"), padding=20)]
        ...
        for case in cases:
            ...
            items.append(
                Container(
                    content=ListTile(
                        leading=Icon(Icons.FOLDER, color=Colors.BLUE_400),
                        title=Row(
                            [
                        ...
                        subtitle=Column(
                            [
                                Text(f"被相続人: {case['deceased_name']}", color="onSurfaceVariant"),
                                Text(f"更新: {case['last_updated_at']} | 次: {case['description'] or 'なし'}", size=12, color="outline"),
                            ],
                            spacing=2,
                        ),
                    ...
                    border_radius=8,
                    padding=5,
                    border=border.only(bottom=border.BorderSide(1, color="outlineVariant")),
                )
            )
        return items
    ...
    def _get_capacity_items(self):
        ...
        items = []
        for d in data:
            color = Colors.RED_400 if d["total_incomplete_tasks"] > 5 else None
            items.append(
                ListTile(
                    title=Text(f"{d['name']} ({d['role']})", weight=FontWeight.BOLD, color=color, size=14),
    ...
    def _create_main_list_view_column(self):
        return Column(
            controls=[
                Container(content=Row([Text("案件一覧", size=20, weight=FontWeight.BOLD)]), padding=dict(left=10)),
    ...
    app_bar = AppBar(
        ...
        actions=[
            IconButton(
                icon=Icons.REFRESH,
                tooltip="画面を更新",
                on_click=lambda e: page.go("/") # 現在のページをリロードする形
            ),
        ]
    )
